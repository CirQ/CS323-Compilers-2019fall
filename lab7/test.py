...
class SymbolTable(ctypes.Structure):
    ...
    def __init__(self):
        ...
        self.lib.symtab_delete.restype = ctypes.c_int32
    ...
    def delete(self, key):
        key = self.safe_key(key)
        r = self.lib.symtab_delete(self.this, key)
        return int(r)


class SymbolTableTest(unittest.TestCase):
    def test_suite_01(self):
        st = SymbolTable()
        st.insert('10', 10)
        st.insert('20', 20)
        ...
        self.assertEqual(st.delete('10'), 1)
        self.assertEqual(st.delete('10'), 0)

    def test_suite_02(self):
        st = SymbolTable()
        size = 10000
    ...
    def test_suite_03(self):
        ...
        for k in random.sample(ks, trial):
            del tab[k]
            self.assertEqual(st.delete(k), 1)
